...
const PostAutomationView: React.FC = () => {
              ...
              <AutomationEditor
                ...
                onDelete={selectedMedia?.automation_id ? async (id) => {
                  try {
                    ...
                    const res = await authenticatedFetch(`${((globalThis as any).__DM_PANDA_API_BASE_URL__ || import.meta.env.VITE_API_BASE_URL)}/api/instagram/automations/${id}?account_id=${activeAccountID}&type=${backendType}`, {
                      method: 'DELETE'
                    });
                    if (res.ok) {
                      showSuccess('Post automation deleted successfully.');
                      setEditorDirty(false);
                      setHasUnsavedChanges(false);
  ...
  return (
    <div className="relative h-full">
      <MediaSection
        key={`post-media-${activeAccountID || 'none'}-${mediaRefreshKey}`}
