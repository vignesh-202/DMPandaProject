        ...
        <Input
          ...
          className={cn(
            ...
            showMatchIcon && value && (isMatch
              ? "border-green-500 focus:border-green-500 focus:ring-green-500/20 bg-green-50/30 dark:bg-green-900/10"
              : "border-red-500 focus:border-red-500 focus:ring-red-500/20 bg-red-50/30 dark:bg-red-900/10")
          )}
        />
...
const AccountSettingsView = () => {
  ...
  const handleUnlinkIG = async (accountID: string) => {
    ...
    resetMessages();
    try {
      const response = await authenticatedFetch(`${import.meta.env.VITE_API_BASE_URL}/api/account/ig-accounts/unlink/${accountID}`, {
        method: 'POST'
      });
      if (response.ok) {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 lg:space-y-10 max-w-4xl mx-auto select-text">
      ...
      <section className="space-y-3 sm:space-y-4">
        <h3 className="text-lg sm:text-xl font-semibold text-foreground flex items-center gap-2">
          <span className="w-1 h-6 bg-gradient-to-b from-ig-purple to-ig-blue rounded-full"></span>
          Profile Information
        </h3>
        <Card className="max-w-none border border-content shadow-md hover:shadow-lg transition-shadow duration-300">
          <form onSubmit={handleInfoSubmit} className="p-5 sm:p-6 space-y-5 sm:space-y-6">
            ...
            <Button type="submit" disabled={isSubmittingInfo} className="px-6 sm:px-8 h-12 rounded-xl bg-gradient-to-r from-ig-purple to-ig-blue hover:from-primary hover:to-ig-blue text-white font-semibold w-full sm:w-auto shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 transition-all duration-300">
      ...
      <section className="space-y-3 sm:space-y-4">
        <h3 className="text-lg sm:text-xl font-semibold text-foreground flex items-center gap-2">
          <span className="w-1 h-6 bg-gradient-to-b from-ig-blue to-ig-purple rounded-full"></span>
          Security
        </h3>
        {hasPassword ? (
          <Card className="max-w-none border border-content shadow-md hover:shadow-lg transition-shadow duration-300">
          ...
          <Card className="max-w-none border border-content shadow-md">
            <div className="p-5 sm:p-6 text-center space-y-4">
              ...
              <Button onClick={() => setShowSetPassword(true)} className="h-12 px-6 sm:px-8 rounded-xl bg-gradient-to-r from-ig-purple to-ig-blue text-white font-semibold shadow-lg shadow-primary/20">Set a Password</Button>
            </div>
          </Card>
        )}
      </section>
      ...
      <section className="space-y-3 sm:space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <h3 className="text-lg sm:text-xl font-semibold text-foreground flex items-center gap-2">
            <span className="w-1 h-6 bg-gradient-to-b from-ig-yellow via-ig-pink to-ig-purple rounded-full"></span>
          ...
          <Button
            ...
            className="rounded-xl h-9 px-3 sm:px-4 border border-slate-200 dark:border-slate-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all flex items-center gap-2 group self-start sm:self-auto"
          >
            <RefreshCw className={cn("h-4 w-4 text-gray-400 group-hover:text-blue-500 transition-colors", isLoadingAccounts && "animate-spin text-blue-500")} />
            <span className="text-xs font-bold uppercase tracking-wider text-gray-500 group-hover:text-gray-900 dark:group-hover:text-white">Refresh</span>
          </Button>
        </div>
          ...
          <div className="p-5 sm:p-6 space-y-5 sm:space-y-6">
            {isLoadingAccounts ? (
                    ...
                    <div className="flex items-center gap-4 w-full">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[11px] sm:text-xs text-muted-foreground uppercase tracking-wider font-semibold">
                          Status: <span className={account.status === 'active' ? 'text-green-600 dark:text-green-400' : 'text-muted-foreground'}>
                            {account.status}
                          </span>
                        </p>
                      </div>
                      ...
                      {account.status === 'active' ? (
                        <>
                          <Button
                            ...
                            className="flex-1 sm:flex-none h-11 border-border hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 hover:border-red-300 dark:hover:border-red-900 text-foreground rounded-xl px-4 transition-all duration-200"
                          >
                        ...
                        <div className="flex gap-2 w-full">
                          ...
                          {account.status === 'unlinked' ? (
                            <Button
                              ...
                              className="flex-1 sm:flex-none h-11 bg-gradient-to-r from-ig-purple to-ig-blue hover:from-primary hover:to-ig-blue text-white shadow-md shadow-primary/20 transition-all duration-300 rounded-xl px-4"
                            >
                          ...
                          <Button
                            ...
                            className="flex-1 sm:flex-none h-10 sm:h-11 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 border-0 rounded-xl font-bold px-4"
                          >
              ...
              <div className="text-center py-8 space-y-4">
                ...
                <p className="text-gray-500 dark:text-gray-400">No Instagram accounts linked yet.</p>
              </div>
            )}
            ...
            <div className="pt-6 border-t border-border">
              ...
              <Button
                ...
                className="w-full mt-4 h-12 bg-gradient-to-r from-ig-blue via-ig-purple to-ig-pink text-white hover:from-ig-blue/90 hover:via-ig-purple/90 hover:to-ig-pink/90 hover:shadow-lg hover:shadow-ig-purple/25 transition-all duration-200 rounded-xl border-0 font-semibold flex items-center justify-center gap-2.5 group relative overflow-hidden"
              >
        ...
        <Card className="border-2 border-red-200 dark:border-red-900/40 bg-red-50/50 dark:bg-red-950/20">
          <div className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-red-600/70 dark:text-red-400/70">Permanently delete your account and all its data.</p>
            </div>
            <div className="flex flex-col gap-2">
              <Button
                variant="destructive"
                onClick={() => setShowDeleteModal(true)}
                className="bg-red-600 hover:bg-red-700 h-12 px-8 rounded-xl border-0 shadow-md shadow-red-500/20 hover:shadow-lg hover:shadow-red-500/30 transition-all duration-200"
              >
                Delete Account
              </Button>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <Card className="w-full max-w-md p-8 shadow-2xl border-0 bg-white dark:bg-gray-900 rounded-3xl relative">
            <button onClick={() => setShowDeleteModal(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600">
              <X className="h-6 w-6" />
            </button>
            <h3 className="text-2xl font-bold text-red-600 dark:text-red-400 mb-2 text-center">Delete Your Account?</h3>
            <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-2xl border border-red-100 dark:border-red-900/30 mb-6">
              <p className="text-sm text-red-800 dark:text-red-400 font-bold mb-2">⚠️ Warning: Irreversible Actions</p>
              <ul className="text-xs text-red-700 dark:text-red-500 space-y-2 text-left list-disc list-inside font-medium leading-relaxed">
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col gap-3 pt-2">
                <Button
                  className="bg-red-600 hover:bg-red-700 text-white h-12 rounded-xl text-lg font-bold"
                ...
                <Button
                  ...
                  disabled={isDeleting}
                  className="h-12 text-gray-500 hover:text-gray-800 dark:hover:text-gray-200"
                >
                  Cancel
                </Button>
              </div>
      ...
      {showUnlinkConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <Card className="w-full max-w-md p-8 shadow-2xl border-0 bg-white dark:bg-gray-900 rounded-3xl relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1.5 bg-red-500"></div>
            <button onClick={() => setShowUnlinkConfirm(null)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
              <X className="h-6 w-6" />
            </button>

            <div className="flex flex-col items-center text-center space-y-4">
              <div className="w-16 h-16 bg-red-50 dark:bg-red-900/20 rounded-full flex items-center justify-center">
                <Unlink className="h-8 w-8 text-red-600 dark:text-red-400" />
              </div>

              <div className="space-y-2">
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Unlink Account?</h3>
                <div className="p-4 bg-orange-50 dark:bg-orange-900/10 rounded-2xl border border-orange-100 dark:border-orange-900/20">
                  ...
                  <ul className="text-xs text-orange-700 dark:text-orange-500 mt-2 space-y-1 text-left list-disc list-inside">
              ...
              <div className="flex flex-col w-full gap-3 pt-4">
                <Button
                  className="w-full bg-red-600 hover:bg-red-700 text-white h-12 rounded-xl font-bold shadow-lg shadow-red-500/20"
                  onClick={async () => {
                ...
                <Button
                  ...
                  className="w-full h-12 text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200"
                >
                  Keep Account Linked
                </Button>
              </div>
      ...
      {showDeleteIGConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <Card className="w-full max-w-md p-8 shadow-2xl border-0 bg-white dark:bg-gray-900 rounded-3xl relative overflow-hidden">
            ...
            <button onClick={() => { setShowDeleteIGConfirm(null); setDeleteIGPassword(''); }} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
              <X className="h-6 w-6" />
            </button>
            ...
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <div className="space-y-2">
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Delete Data Record?</h3>
                <div className="p-4 bg-red-50 dark:bg-red-900/10 rounded-2xl border border-red-100 dark:border-red-900/20">
                  <p className="text-sm text-red-800 dark:text-red-400 font-bold mb-2 text-left">
              ...
              <div className="w-full space-y-4 pt-2">
                <div className="space-y-2">
                  <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">Verify with your password to continue</p>
                  <Input
                    type="password"
      ...
      {showSetPassword && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <Card className="w-full max-w-md p-8 shadow-2xl border-0 bg-white dark:bg-gray-900 rounded-3xl relative overflow-hidden">
            ...
            <button onClick={() => { setShowSetPassword(false); setNewPassword(''); setConfirmPassword(''); }} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
              <X className="h-6 w-6" />
            </button>

            <div className="flex flex-col items-center text-center space-y-4">
              <div className="w-16 h-16 bg-purple-50 dark:bg-purple-900/20 rounded-full flex items-center justify-center">
                <LinkIcon className="h-8 w-8 text-purple-600 dark:text-purple-400" />
              </div>

              <div className="space-y-2">
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Set a Password</h3>
              ...
              <form onSubmit={handleSetPasswordSubmit} className="w-full space-y-6 pt-2">
                ...
                <div className="flex flex-col w-full gap-3">
                  <Button
                    type="submit"
                    className="w-full bg-purple-600 hover:bg-purple-700 text-white h-12 rounded-xl font-bold shadow-lg shadow-purple-500/20"
