...
const InsightsView: React.FC = () => {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 px-3 sm:px-4 md:px-6 lg:px-8">
      <div className="rounded-[30px] border border-border bg-[radial-gradient(circle_at_top_left,rgba(64,93,230,0.16),transparent_28%),radial-gradient(circle_at_bottom_right,rgba(245,96,64,0.12),transparent_24%),linear-gradient(135deg,rgba(255,255,255,0.98),rgba(248,250,252,0.96))] p-4 shadow-sm dark:bg-[radial-gradient(circle_at_top_left,rgba(64,93,230,0.20),transparent_28%),radial-gradient(circle_at_bottom_right,rgba(245,96,64,0.16),transparent_24%),linear-gradient(135deg,rgba(23,23,23,0.98),rgba(10,10,10,0.96))] sm:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div className="flex items-start gap-3 sm:gap-4 min-w-0">
            {profilePicture ? (
              <img src={profilePicture} alt={summary.username || activeAccount?.username || 'Instagram account'} className="h-14 w-14 sm:h-16 sm:w-16 rounded-full border-2 border-white/70 object-cover shadow-lg dark:border-white/10 shrink-0" />
            ) : (
            ...
            <div className="min-w-0 flex-1">
              ...
              <h2 className="mt-3 text-xl sm:text-2xl font-bold text-foreground break-all">{summary.username || activeAccount?.username || 'Insights'}</h2>
              <p className="text-sm text-muted-foreground truncate">{summary.name || activeAccount?.name || 'Instagram Business account'}</p>
              {summary.biography ? (
                <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground whitespace-pre-wrap break-words">{summary.biography}</p>
              ) : null}
          ...
          <div className="space-y-3">
            <div className="inline-flex flex-wrap rounded-2xl bg-muted p-1">
              {PERIOD_OPTIONS.map((option) => (
                <button
                  key={option.value}
                  type="button"
                  onClick={() => setPeriod(option.value)}
                  className={`rounded-xl px-3 sm:px-4 py-2 text-xs font-bold transition-all ${period === option.value ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
                >
                  {option.label}
                </button>
              ))}
