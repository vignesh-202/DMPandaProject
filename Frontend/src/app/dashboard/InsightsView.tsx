...
const MetricCard = ({
  ...
  <Card variant="instagram" className="border border-content shadow-xs">
...
const InsightsView: React.FC = () => {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 px-3 sm:px-4 md:px-6 lg:px-8">
      <div className="rounded-2xl border border-border bg-card p-4 shadow-xs sm:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div className="flex items-start gap-3 sm:gap-4 min-w-0">
            {profilePicture ? (
              <img src={profilePicture} alt={summary.username || activeAccount?.username || 'Instagram account'} className="h-14 w-14 sm:h-16 sm:w-16 rounded-full border border-border object-cover shadow-xs shrink-0" />
            ) : (
              <div className="flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center rounded-full bg-muted text-muted-foreground shrink-0">
                <ImageIcon className="h-6 w-6" />
              </div>
            )}
            <div className="min-w-0 flex-1">
              <div className="inline-flex rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-semibold text-primary">
                Instagram Insights
              </div>
              <h2 className="mt-2 text-xl sm:text-2xl font-bold text-foreground break-all">{summary.username || activeAccount?.username || 'Insights'}</h2>
              <p className="text-sm text-muted-foreground truncate">{summary.name || activeAccount?.name || 'Instagram Business account'}</p>
              {summary.biography ? (
                <p className="mt-2.5 max-w-3xl text-sm leading-relaxed text-muted-foreground whitespace-pre-wrap break-words">{summary.biography}</p>
              ) : null}
              <div className="mt-3.5 flex flex-wrap gap-2">
                <span className="rounded-full bg-muted/60 px-3 py-1 text-xs font-medium text-foreground">{formatNumber(summary.followers)} followers</span>
                <span className="rounded-full bg-muted/60 px-3 py-1 text-xs font-medium text-foreground">{formatNumber(summary.following)} following</span>
                <span className="rounded-full bg-muted/60 px-3 py-1 text-xs font-medium text-foreground">{formatNumber(summary.media_count)} media</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="inline-flex flex-wrap rounded-xl bg-muted p-1">
              {PERIOD_OPTIONS.map((option) => (
                <button
                  ...
                  className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition-all ${period === option.value ? 'bg-card text-foreground shadow-xs' : 'text-muted-foreground hover:text-foreground'}`}
                >
                  {option.label}
                </button>
              ))}
            ...
            {summary.website ? (
              <a href={summary.website} target="_blank" rel="noreferrer" className="block text-sm font-medium text-primary underline-offset-4 hover:underline break-all">
                {summary.website}
              </a>
            ) : null}
          </div>
