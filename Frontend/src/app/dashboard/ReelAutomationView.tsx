            ...
            <section className="space-y-6 rounded-[28px] border border-content bg-card p-4 shadow-sm sm:space-y-8 sm:rounded-[34px] sm:p-6 lg:rounded-[40px] lg:p-8 xl:min-h-0">
              <AutomationEditor
                ...
                onDelete={selectedMedia?.automation_id ? async (id) => {
                  try {
                    const res = await authenticatedFetch(`${((globalThis as any).__DM_PANDA_API_BASE_URL__ || import.meta.env.VITE_API_BASE_URL)}/api/instagram/automations/${id}?account_id=${activeAccountID}&type=reel`, {
                      method: 'DELETE'
                    });
                    if (res.ok) {
                      showSuccess('Reel automation deleted successfully.');
                      setEditorDirty(false);
                      setHasUnsavedChanges(false);
